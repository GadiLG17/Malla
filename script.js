document.addEventListener("DOMContentLoaded", () => {
  ...
  for (const area in cursos) {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      listaCursos.forEach(item => {
        ...
        div.setAttribute('data-creditos', '4');
        const req = requisitos[sigla];
        if (req) div.setAttribute('data-requiere', req.join(','));

        const tooltip = document.createElement('div');
        tooltip.className = 'tooltip';
        tooltip.textContent = `Créditos: 4` + (req ? ` | Requiere: ${req.join(', ')}` : '');
        div.textContent = `${sigla} - ${nombre}`;
  ...
  document.querySelectorAll('.curso').forEach(curso => {
    ...
    // Hover de requisitos visual eliminado para evitar conflictos visuales
  });
});
